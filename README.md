...
* **Dynamic Audio Feedback:** Built-in audio engine modulates pitch based on array values and triggers distinct sound effects for array reads versus array modifications.
* **Educational Context:** Algorithms feature built-in, dynamic word-wrapping descriptions to explain their logic directly within the UI.
* **Fully Responsive UI:** The interface dynamically recalculates geometry and text formatting, ensuring perfect layouts across different window sizes.
...
### Prerequisites
You do not need to install Raylib manually; CMake will fetch it automatically. You only need:
...
1. Clone the repository:
   ```bash
   git clone https://github.com/javiiervm/AlgoTrace.git
   cd AlgoTrace
   ```
...
* **UP / DOWN ARROWS:** Increase or decrease the master volume.
...
├── assets/
│   ├── font.ttf                 # UI Font 
│   ├── beep.wav                 # Audio for reading elements
│   └── swap.wav                 # Audio for modifying elements
├── include/
...
├── templates/                   # Templates to add new algorithms easily
│   ├── TemplateSort.cpp                 
│   └── TemplateSort.hpp
...
AlgoTrace is built with a **Factory Pattern** and an **Interface-driven design**. You do not need to understand Raylib, the UI code, or the audio engine to add a new algorithm. Just follow these 4 simple steps:

### Step 1: Create your files

Create a new header (`.hpp`) in the `include/` folder and a source file (`.cpp`) in the `lib/` folder.

* *Tip: It is recommended that you copy the structure from `templates/TemplateSort.hpp` and `templates/TemplateSort.cpp` for better compatibility.*

### Step 2: Inherit from `Algorithm`

Your class must inherit from the `Algorithm` base class and implement its pure virtual methods. Make sure to include the `is_modifying` flag for the audio engine.
...
private:
    ...
    bool is_modifying = false; // Required for dynamic audio
...
public:
    ...
    std::string getDescription() const override { return "A brief explanation of the logic."; }
...
*Audio Note:* Remember to set `is_modifying = true;` during the exact step where your algorithm swaps or modifies array elements to trigger the correct sound effect.
